...
from internvl.train.dataset import (
    ...
    read_frames_gif,
    preprocess_phi3,
    preprocess_internvl2_5_world_token,
)
...
class ModelArguments:
    ...
    use_world_token: bool = field(
        default=False, metadata={"help": "Set to True to use the world token."}
    )
    # world_token_numbers: int = field(
    #     default=10, metadata={"help": "number queries for 3d object detection"}
    # )
    agent_token_numbers: int = field(
        default=0, metadata={"help": "number queries for 3d object detection"}
    )
    gp_token_numbers: int = field(
        default=0, metadata={"help": "number queries for goal point"}
    )
    occ_token_numbers: int = field(
        default=0, metadata={"help": "number queries for occ"}
    )
    dream_world: bool = field(
        default=False, metadata={"help": "Set to True to pred future info."}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        number_world_tokens=10,
        number_dream_tokens=0,
        ...
        # add world token
        self.number_world_tokens = number_world_tokens
        self.number_dream_tokens = number_dream_tokens
        ...
        self.point_cloud_range = [0, -25.0, -4.0, 50.0, 25.0, 4.0]
    ...
    def get_preprocess_function(self):
        ...
        elif self.template_name == "internvl2_5_world_token":
            preprocess_function = preprocess_internvl2_5_world_token
        else:
    ...
    def preprocess_ego_status_hist_traj(self, ego_statuses):
        driving_command = torch.tensor(ego_statuses["driving_command"])
        ego_acceleration = torch.tensor(ego_statuses["ego_acceleration"], dtype=torch.float32)
        ego_pos = torch.tensor(ego_statuses["ego_pose"], dtype=torch.float32)
        ego_velocity = torch.tensor(ego_statuses["ego_velocity"], dtype=torch.float32)
        ego_status = torch.cat([driving_command, ego_acceleration, ego_pos, ego_velocity])
        hist_traj = torch.tensor([
            [float(d['x']), float(d['y']), float(d['heading'])]
            for d in ego_statuses["history_trajectory"]
        ], dtype=torch.float32)
        return ego_status, hist_traj


    def multi_modal_multi_image_get_item_world_token(self, data_item):
        # with world query
        # Build transformation function
        transform = self.get_transform()

        lidar = self.get_occ(data_item, 0)

        future_idx = int(os.environ.get("future_idx", 1))
        lidar_dream = self.get_occ(data_item, future_idx)
        ...
        for image_path in data_item["image"]:
            # Merge the image path
            orig = image_path
            new_prefix = "/path/to/new/prefix/which/contains/sensor_blobs/"

            if "sensor_blobs" in orig:
                before, after = orig.split("sensor_blobs", 1)
                image_path = f"{new_prefix}sensor_blobs{after}"
            else:
                raise RuntimeError("sensor_blobs not found")
        ...
        for idx, conversation in enumerate(data_item["conversations"]):
            if conversation["from"] == "human":
                data_item["conversations"][idx]["value"] += "\n" + "".join(
                    ["<world>"] * self.number_world_tokens
                )

                if self.number_dream_tokens > 0:
                    data_item["conversations"][idx]["value"] += "\n" + "".join(
                        ["<dream>"] * self.number_dream_tokens
                    )

                image_cnt = data_item["conversations"][idx]["value"].count("<image>")
                if image_cnt == 0:
                    data_item["conversations"][idx]["value"] = "".join(["<image>"] * num_image + [data_item["conversations"][idx]["value"]])
        ...
        annotations = data_item["annotations"][0]
        agent_states = torch.tensor(annotations.get("agent_states", []), dtype=torch.float)
        agent_labels = torch.tensor(annotations.get("agent_labels", []), dtype=torch.float)
        agent_names = annotations.get("agent_names", [])

        class2id = {name: i for i, name in enumerate(class_names)}
        agent_labels_int = torch.tensor(
            [class2id[name] for name in agent_names], dtype=torch.long
        )  # (N,)
        agent_labels_int[agent_labels < 0] = -1

        # Predict future time step t
        annotations = data_item["annotations"][future_idx]
        agent_states_ft = torch.tensor(annotations.get("agent_states", []), dtype=torch.float)
        agent_labels_ft = torch.tensor(annotations.get("agent_labels", []), dtype=torch.float)
        agent_names_ft = annotations.get("agent_names", [])
        class2id = {name: i for i, name in enumerate(class_names)}
        agent_labels_int_ft = torch.tensor([class2id[name] for name in agent_names_ft], dtype=torch.long)  # (N,)
        agent_labels_int_ft[agent_labels_ft < 0] = -1

        # process ego_status and hist_traj
        ego_status, hist_traj = self.preprocess_ego_status_hist_traj(data_item["ego_statuses"][0])
        ...
        ret = dict(
            ...
            agent_labels=agent_labels_int,
            agent_states_ft= agent_states_ft,
            agent_labels_ft=agent_labels_int_ft,
            trajectory=torch.tensor(data_item["future_trajectory"])[...,:2],    # TODO: only use xy
            goalpoint=torch.tensor(data_item["future_trajectory"][-1])[:2],
            ego_status=ego_status,
            hist_traj=hist_traj,
            lidar_gt = lidar,
            lidar_gt_dream = lidar_dream,

        )
        return ret
    
    def point_to_segment_distance_torch(self, p, a, b):
        """
        Calculate the shortest distance from point p to segment ab (vectorized support)
        p: [K, 2]
        a: [N, 2]
        b: [N, 2]
        Returns: [K, N] each point to each segment distance
        """
        ab = b - a  # [N, 2]
        ap = p.unsqueeze(1) - a.unsqueeze(0)  # [K, N, 2]
        ab_norm_sq = (ab ** 2).sum(-1, keepdim=True).clamp(min=1e-6)  # [N, 1]
        t = (ap * ab.unsqueeze(0)).sum(-1, keepdim=True) / ab_norm_sq.unsqueeze(0)  # [K, N, 1]
        t = t.clamp(0.0, 1.0)
        closest = a.unsqueeze(0) + t * ab.unsqueeze(0)  # [K, N, 2]
        dist = torch.norm(p.unsqueeze(1) - closest, dim=-1)  # [K, N]
        return dist


    def filter_bboxes_near_traj(self, bboxes, traj, threshold=16.0):
        """
        bboxes: [M, 7] -> (x, y, z, w, l, h, yaw)
        traj: [N, 3]   -> (x, y, yaw)
        threshold: float, distance range on both sides of the trajectory (unit: meters)

        return: valid_mask [M] (bool), indicating which bboxes are selected
        """
        # Extract trajectory xy coordinates
        traj_xy = traj[:, :2]
        # Extract bbox center points
        centers = bboxes[:, :2]  # [M, 2]
        ...
        dist_matrix = self.point_to_segment_distance_torch(centers, a, b)  # [M, N-1]
        min_dist, _ = torch.min(dist_matrix, dim=1)  # [M]

        # mask
        valid_mask = min_dist <= threshold
        return valid_mask

    def get_occ(self, data_item, idx):
        lidar_path_base = data_item['lidar_path'][idx]

        new_prefix = "/path/to/new/prefix/which/contains/sensor_blobs/"
        lidar_path = re.sub(r'^.*?(?=sensor_blobs)', new_prefix, lidar_path_base)
        ...
        valid = (
            (pts[:, 0] >= self.point_cloud_range[0])
            & (pts[:, 0] < self.point_cloud_range[3])
            & (pts[:, 1] >= self.point_cloud_range[1])
            & (pts[:, 1] < self.point_cloud_range[4])
            & (pts[:, 2] >= self.point_cloud_range[2])
            & (pts[:, 2] < self.point_cloud_range[5])
        )
        valid_pts = pts[valid]
        voxel_coords = (
            (
        ...
        traj_mask = self.create_mask_with_opencv(traj[...,:2], distance=10, resolution=0.5)
        traj_mask = np.repeat(traj_mask[:, :, np.newaxis], 16, axis=2)
        target[traj_mask==1] = 0

        return target
    
    def create_mask_with_opencv(self, points, distance=2, resolution=0.1):
        ...
        polygon_pixels = np.array([[
                                    [int((y - (self.point_cloud_range[1])) / resolution), int((x - self.point_cloud_range[0]) / resolution)] 
            ...
            try:
                ...
                if "image" in data_item and len(data_item["image"]) != 0:
                    if type(data_item["image"]) == list:
                        ret = self.multi_modal_multi_image_get_item_world_token(
                            data_item
                        )
                    else:
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            number_world_tokens=number_world_tokens,  # add world token numbers
            number_dream_tokens=number_dream_tokens,
            # hyperparameters for packed training
...
def safe_init_weights(m: nn.Module):
    if isinstance(m, nn.Conv2d):
        orig_dtype = m.weight.dtype
        m.weight.data = m.weight.data.float()  # Convert to float32 for initialization
        nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        m.weight.data = m.weight.data.to(orig_dtype)  # Convert back to original dtype
        if m.bias is not None:
            m.bias.data = m.bias.data.float()
            nn.init.constant_(m.bias, 0)
            m.bias.data = m.bias.data.to(orig_dtype)
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        config = InternVLChatConfig_WM.from_pretrained(model_args.model_name_or_path)
        ...
        # add world token numbers
        config.agent_token_numbers = model_args.agent_token_numbers
        config.gp_token_numbers = model_args.gp_token_numbers
        config.occ_token_numbers = model_args.occ_token_numbers
        config.dream_world = model_args.dream_world
        config.world_token_numbers = model_args.agent_token_numbers + model_args.gp_token_numbers + model_args.occ_token_numbers
        ...
        internvl_chat_config = InternVLChatConfig_WM(
        ...
        model = InternVLChatModel_WM(internvl_chat_config, vision_model, llm)
    ...
    if model_args.use_world_token and training_args.do_train:
        model._agent_head.apply(safe_init_weights)
        model._gp_head.apply(safe_init_weights)
        model._wm_encoder.apply(safe_init_weights)
        model._occ_head.apply(safe_init_weights)
        model.ego_status_encoder.apply(safe_init_weights)
        model.reduce_fused_feature.apply(safe_init_weights)
        model.hist_traj_encoder.apply(safe_init_weights)

    if model_args.dream_world and training_args.do_train:
        model._wm_encoder_dream.apply(safe_init_weights)
        model._occ_head_dream.apply(safe_init_weights)
        model._agent_head_dream.apply(safe_init_weights)
    ...
    if num_new_tokens > 0:  # average initialization
    ...
    train_dataset = build_datasets(
        ...
        number_world_tokens=config.world_token_numbers,
        number_dream_tokens=model_args.occ_token_numbers + model_args.agent_token_numbers if model_args.dream_world else 0
    )
