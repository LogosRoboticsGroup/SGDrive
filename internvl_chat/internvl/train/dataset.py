...
def preprocess_internlm(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        temp_len = len(tokenizer(info).input_ids) - 1  # Remove the <s> token from the tokenizer.
...
def preprocess_internvl2_5_world_token(
    template_name,
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
    num_image_token_list: list,
    ...
    if not text_only:
        ...
        for conversation in conversations:
            if conversation["from"] == "human":
                ...
                dream_cnt = conversation["value"].count(
                    "<dream>"
                )  # default need to modify  should be same with world query numbers
