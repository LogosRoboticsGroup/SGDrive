...
from .sgdrive_dit import LightningDiT
...
@dataclass
class SGDriveDiffusionPlannerConfig(PretrainedConfig):
    """A refined configuration for the SGDriveDiffusionPlanner."""
...
class SGDriveDiffusionPlanner(nn.Module):
    config_class = SGDriveDiffusionPlannerConfig

    def __init__(self, config: SGDriveDiffusionPlannerConfig):
        super().__init__()
        self.config = config
        ...
        self.fusion_projector = nn.Linear(
            config.input_embedding_dim * 3, config.input_embedding_dim
        )
        # if config.vlm_size == "large":
        #     self.agent_encoder = nn.Linear(3584, config.input_embedding_dim)
        #     self.occ_encoder = nn.Linear(3584, config.input_embedding_dim)
        #     self.gp_encoder = nn.Linear(3584, config.input_embedding_dim)
        #     self.dream_agent_encoder = nn.Linear(3584, config.input_embedding_dim)
        #     self.dream_occ_encoder = nn.Linear(3584, config.input_embedding_dim)
        # else:
        #     self.agent_encoder = nn.Linear(1536, config.input_embedding_dim)
        #     self.occ_encoder = nn.Linear(1536, config.input_embedding_dim)
        #     self.gp_encoder = nn.Linear(1536, config.input_embedding_dim)
        #     self.dream_agent_encoder = nn.Linear(1536, config.input_embedding_dim)
        #     self.dream_occ_encoder = nn.Linear(1536, config.input_embedding_dim)
        ...
        output_dim = (
            2 * config.action_dim
            if (config.sampling_method == "flow" and config.flow_cfg.mean_variance_net)
        ...
        if config.add_pos_embed:
            self.position_embedding = nn.Embedding(
                config.max_seq_len, config.input_embedding_dim
            )
    ...
    def _init_ddpm_sampler(self, cfg: DDPMConfig):
        ...
        ddpm_alphas_cumprod_prev = torch.cat(
            [torch.tensor([1.0]), ddpm_alphas_cumprod[:-1]]
        )
        self.register_buffer("ddpm_alphas_cumprod_prev", ddpm_alphas_cumprod_prev)

        self.register_buffer(
            "ddpm_sqrt_alphas_cumprod", torch.sqrt(ddpm_alphas_cumprod)
        )
        self.register_buffer(
            "ddpm_sqrt_one_minus_alphas_cumprod", torch.sqrt(1.0 - ddpm_alphas_cumprod)
        )
        self.register_buffer(
            "ddpm_sqrt_recip_alphas_cumprod", torch.sqrt(1.0 / ddpm_alphas_cumprod)
        )
        self.register_buffer(
            "ddpm_sqrt_recipm1_alphas_cumprod",
            torch.sqrt(1.0 / ddpm_alphas_cumprod - 1.0),
        )

        ddpm_var = (
            ddpm_betas * (1.0 - ddpm_alphas_cumprod_prev) / (1.0 - ddpm_alphas_cumprod)
        )
        self.register_buffer("ddpm_var", ddpm_var)
        self.register_buffer(
            "ddpm_logvar_clipped", torch.log(ddpm_var.clamp(min=1e-20))
        )

        self.register_buffer(
            "ddpm_mu_coef1",
            ddpm_betas
            * torch.sqrt(ddpm_alphas_cumprod_prev)
            / (1.0 - ddpm_alphas_cumprod),
        )
        self.register_buffer(
            "ddpm_mu_coef2",
            (1.0 - ddpm_alphas_cumprod_prev)
            * torch.sqrt(ddpm_alphas)
            / (1.0 - ddpm_alphas_cumprod),
        )
    ...
    def _init_ddim_sampler(self, cfg: DDIMConfig):
        ...
        self.register_buffer(
            "ddim_t_schedule", ddim_t.long()
        )  # Die originalen Zeitpunkte

        ddim_alphas = (
            self.ddpm_alphas_cumprod[self.ddim_t_schedule].clone().to(torch.float32)
        )
    ...
    def _init_grpo(self, cfg: GRPOConfig):
        ...
        try:
            state_dict = torch.load(
                cfg.reference_policy_checkpoint, map_location="cpu"
            )["state_dict"]
            ...
            for k, v in state_dict.items():
                ...
                if k2 in model_dict and v.shape == model_dict[k2].shape:
                    ...
                    print(
                        f"Skip loading '{k}' → '{k2}' (checkpoint shape {tuple(v.shape)} vs model shape {tuple(model_dict.get(k2, v).shape)})"
                    )
            self.load_state_dict(filtered_ckpt, strict=True)
        except FileNotFoundError:
            print(
                f"Warning: GRPO checkpoint not found at {cfg.reference_policy_checkpoint}. Skipping loading."
            )
    ...
    def set_frozen_modules_to_eval_mode(self):
        ...
        if self.training:
            ...
            if not self.config.tune_projector:
                ...
                # self.occ_encoder.eval()
                # self.gp_encoder.eval()
    ...
    def sample_time(self, batch_size, device, dtype):
        ...
        if self.config.sampling_method == "flow":
            ...
            return (
                self.config.flow_cfg.noise_s - sample
            ) / self.config.flow_cfg.noise_s
        elif self.config.sampling_method in ["ddpm", "ddim"]:
            return torch.randint(
                0, self.ddpm_num_train_timesteps, (batch_size,), device=device
            ).long()
        else:
            raise ValueError(
    ...
    def p_mean_variance(
        ...
        if use_ori:
            ...
            vl_features_mean = (
                vl_features.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
            )
            ...
            if self.config.sampling_method == "ddpm":
                x_recon = (
                    self.extract(self.ddpm_sqrt_recip_alphas_cumprod, t, x.shape) * x
                    - self.extract(self.ddpm_sqrt_recipm1_alphas_cumprod, t, x.shape)
                    * pred_noise
                )
            elif self.config.sampling_method == "ddim":
                alpha_t = self.extract(self.ddim_alphas, index, x.shape)
                sqrt_one_minus_alpha_t = self.extract(
                    self.ddim_sqrt_one_minus_alphas, index, x.shape
                )
                x_recon = (x - sqrt_one_minus_alpha_t * pred_noise) / (alpha_t**0.5)
            else:
                raise ValueError(
                    f"p_mean_variance not supported for method: {self.config.sampling_method}"
                )
            ...
            if self.config.sampling_method == "ddpm":
                model_mean = (
                    self.extract(self.ddpm_mu_coef1, t, x.shape) * x_recon
            ...
            #     print(f"p_vl_features[0]: {vl_features[0].shape}") #676,384?
            #     print(f"p_vl_features[1]: {vl_features[1].shape}") #676,384?
            #     print(f"p_vl_features[2]: {vl_features[2].shape}") #676,384?
            # agent_embeds_mean = (
            #         vl_features[1].mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
            #     )
            # occ_embeds_mean = (
            #     vl_features[0].mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
            # )
            # gp_embeds_mean = (
            #     vl_features[2].mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
            # )
            # dream_agent_embeds_mean = (
            #         vl_features[4].mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
            #     )
            # dream_occ_embeds_mean = (
            #     vl_features[3].mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
            # )
            # if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
            #     print(f"agent_embeds_mean: {agent_embeds_mean.shape}") #16 8 384
            #     print(f"occ_embeds_mean: {occ_embeds_mean.shape}") # 16 8 384
            #     print(f"gp_embeds_mean: {gp_embeds_mean.shape}") # 16 8 384

            fused_input = self.fusion_projector(
                torch.cat((his_traj_features, vl_features_mean, action_features), dim=2)
            )
            # mean_tensors = [occ_embeds_mean, agent_embeds_mean, gp_embeds_mean, dream_occ_embeds_mean, dream_agent_embeds_mean]
            # mean_embeds = torch.cat(mean_tensors, dim=2)

            # all_features = [his_traj_features, mean_embeds, action_features]
            # fused_input = self.fusion_projector(torch.cat(all_features, dim=2))

            #
            # vl_features = torch.cat(vl_features, dim=1)
    ...
    def forward(
        self, vl_features: torch.Tensor, action_input: BatchFeature
    ) -> BatchFeature:
        """
        ...
        if use_ori:
            ...
            if self.config.sampling_method == "flow":
                ...
                if hasattr(self, "position_embedding"):
                    pos_ids = torch.arange(
                        action_features.shape[1], device=gt_actions.device
                    )
                    action_features += self.position_embedding(pos_ids)

                vl_embeds_mean = (
                    vl_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
                )
                fused_input = self.fusion_projector(
                ...
                model_output = self.model(
                    fused_input, vl_embeds, ego_status_features, t_discrete
                )
                ...
                noisy_actions = (
                    self.extract(
                        self.ddpm_sqrt_alphas_cumprod, t_discrete, gt_actions.shape
                    )
                    * gt_actions
                    + self.extract(
                        self.ddpm_sqrt_one_minus_alphas_cumprod,
                        t_discrete,
                        gt_actions.shape,
                    )
                    * noise
            ...
            # agent_embeds = self.agent_encoder(vl_features[1])
            # occ_embeds = self.occ_encoder(vl_features[0])
            # gp_embeds = self.gp_encoder(vl_features[2])
            # dream_agent_embeds = self.dream_agent_encoder(vl_features[4])
            # dream_occ_embeds = self.dream_occ_encoder(vl_features[3])
            vl_embeds = self.feature_encoder(torch.cat(vl_features, dim=1))
            # vl_embeds = torch.cat((occ_embeds, agent_embeds, gp_embeds, dream_occ_embeds, dream_agent_embeds), dim=1) #16,676,384
            ...
            if self.config.sampling_method == "flow":
                ...
                # agent_embeds_mean = (
                #     agent_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
                # )
                # occ_embeds_mean = (
                #     occ_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
                # )
                # gp_embeds_mean = (
                #     gp_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
                # )
                # dream_agent_embeds_mean = (
                #     dream_agent_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
                # )
                # dream_occ_embeds_mean = (
                #     dream_occ_embeds.mean(1).unsqueeze(1).repeat(1, self.config.action_horizon, 1)
                # )

                # fused_input = self.fusion_projector(
                #     torch.cat((his_traj_features, agent_embeds_mean, occ_embeds_mean, gp_embeds_mean, action_features), dim=2)
                # )
                fused_input = self.fusion_projector(
                    torch.cat((his_traj_features, vl_embeds_mean, action_features), dim=2)
                )
        ...
        if use_ori:
            ...
            if self.config.sampling_method == "flow":
                ...
                for step in range(self.config.num_inference_steps):
                    ...
                    pred_flow = (
                        pred.chunk(2, dim=-1)[0]
                        if self.config.flow_cfg.mean_variance_net
                        else pred
                    )
                ...
                step_size = (
                    self.config.ddpm_cfg.num_train_timesteps
                    // self.config.num_inference_steps
                )
                timesteps_to_iterate = list(
                    reversed(range(0, self.config.ddpm_cfg.num_train_timesteps, step_size))
                )
                ...
                for i, t_int in enumerate(timesteps_to_iterate):
                    ...
                    if (
                        hasattr(self, "eval_randn_clip_value")
                        and self.eval_randn_clip_value is not None
                    ):
                        noise_sample.clamp_(
                            -self.eval_randn_clip_value, self.eval_randn_clip_value
                        )

                    current_actions = mean + std * noise_sample

                    if (
                        hasattr(self, "final_action_clip_value")
                        and self.final_action_clip_value is not None
                        and i == len(timesteps_to_iterate) - 1
                    ):
                        current_actions.clamp_(
                            -self.final_action_clip_value, self.final_action_clip_value
                        )

            elif self.config.sampling_method == "ddim":
                eval_min_sampling_denoising_std = getattr(
                    self, "eval_min_sampling_denoising_std", 0.0001
                )
            ...
            vl_embeds = self.feature_encoder(torch.cat(vl_features, dim=1))
            # if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
            #     print(f"vl_features[0]: {vl_features[0].shape}") 
            #     print(f"vl_features[1]: {vl_features[1].shape}") #16,625,1536
            #     print(f"vl_features[2]: {vl_features[2].shape}") #16,1,1536

            # agent_embeds = self.agent_encoder(vl_features[1])
            # occ_embeds = self.occ_encoder(vl_features[0])
            # gp_embeds = self.gp_encoder(vl_features[2])
            # dream_agent_embeds = self.agent_encoder(vl_features[4])
            # dream_occ_embeds = self.occ_encoder(vl_features[3])

            # if agent_embeds.shape[1] == 1:
            #     agent_embeds = agent_embeds.squeeze(1)
            #     occ_embeds = occ_embeds.squeeze(1)
            #     gp_embeds = gp_embeds.squeeze(1)
            ...
            if self.config.sampling_method == "flow":
                ...
                #         t_batch,
                #         index_batch,
                #         [agent_embeds,occ_embeds,gp_embeds],
                #         history_embeds,
                #         ego_embeds,
                #         deterministic,
                #     )
    ...
    def sample_chain(
        ...
        ego_status_features = self.ego_status_encoder(ego_status_features)
        ...
        if self.config.sampling_method == "flow":
            ...
            for step in range(self.config.num_inference_steps):
                ...
                fused_input = self.fusion_projector(
                    torch.cat(
                        (his_traj_features, vl_features_mean, action_features), dim=2
                    )
                )

                model_output = self.model(
                    fused_input, vl_features, ego_status_features, t_batch
                )
                pred = self.action_decoder(model_output)
            ...
            if self.config.sampling_method == "ddpm":
                ...
                timesteps = list(
                    reversed(
                        range(0, self.config.ddpm_cfg.num_train_timesteps, step_size)
                    )
                )
            else:
            ...
            for i, t_int in enumerate(timesteps):
                t_batch = self.make_timesteps(B, t_int, device)
                index_batch = (
                    self.make_timesteps(B, i, device)
                ...
                if (
                    hasattr(self, "randn_clip_value")
                    and self.randn_clip_value is not None
                ):
                    noise_sample = noise_sample.clamp_(
                ...
                if (
                    i == len(timesteps) - 1
                    and hasattr(self, "final_action_clip_value")
                    and self.final_action_clip_value is not None
                ):
                    current_actions = current_actions.clamp_(
                        -self.final_action_clip_value, self.final_action_clip_value
                    )
    ...
    def get_logprobs(
        ...
        for key, value in conditioning_embeds.items():
            batched_conditioning[key] = (
                value.unsqueeze(1)
                .repeat(1, num_denoising_steps, *(1,) * (value.ndim - 1))
                .flatten(0, 1)
            )
    ...
    def forward_grpo(
        ...
        discount = discount.view(1, 1, num_denoising_steps).expand(
            B, G, num_denoising_steps
        )
