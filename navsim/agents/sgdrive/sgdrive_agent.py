...
from .sgdrive_diffusion_planner import (
    SGDriveDiffusionPlanner,
    SGDriveDiffusionPlannerConfig,
)


class SGDriveAgent(AbstractAgent):
    def __init__(
        self,
        ...
        if not self.cache_hidden_state and not self.cache_mode:
            ...
            if not self.vlm_path or not self.vlm_type:
                raise ValueError(
                    "In 'no-cache' mode, vlm_path and vlm_type are required."
                )
            self.backbone = SGDriveBackbone(
                model_type=self.vlm_type, checkpoint_path=self.vlm_path, device=device
            )

        if self.dit_type == "large":
            cfg = make_sgdrive_config(
                self.dit_type,
                action_dim=3,
                action_horizon=8,
                grpo=self.grpo,
        ...
        self.action_head = SGDriveDiffusionPlanner(cfg).cuda()
    ...
    def forward(
        self, features: Dict[str, torch.Tensor], targets=None, tokens_list=None
        ...
        if self.cache_hidden_state:
            ...
            agent_hiddent_state = features["agent_hidden_state"].cuda()
            occ_hiddent_state = features["occ_hidden_state"].cuda()
            gp_hiddent_state = features["gp_hidden_state"].cuda()
            dream_agent_hiddent_state = features["dream_agent_hidden_state"].cuda()
            dream_occ_hiddent_state = features["dream_occ_hidden_state"].cuda()
        else:
            if self.backbone is None:
                raise RuntimeError(
                    "Agent is in 'no-cache' mode, but backbone is not initialized."
                )
            ...
            outputs = self.backbone(
                pixel_values_cat, questions, num_patches_list=num_patches_list
            )
        ...
        if agent_hiddent_state.ndim == 2:
            # last_hidden_state = last_hidden_state.unsqueeze(0)
            agent_hiddent_state = features["agent_hidden_state"].unsqueeze(0)
            occ_hiddent_state = features["occ_hidden_state"].unsqueeze(0)
            gp_hiddent_state = features["gp_hidden_state"].unsqueeze(0)
            dream_agent_hiddent_state = features["dream_agent_hidden_state"].unsqueeze(0)
            dream_occ_hiddent_state = features["dream_occ_hidden_state"].unsqueeze(0)

        history_trajectory_reshaped = history_trajectory.view(
            history_trajectory.size(0), -1
        )
        ...
        if self.training and not self.grpo:
            ...
            return self.action_head.forward_grpo(
                torch.cat([s.to(model_dtype) for s in [occ_hiddent_state, agent_hiddent_state, gp_hiddent_state, dream_occ_hiddent_state, dream_agent_hiddent_state]], dim=1), action_inputs, tokens_list
            )
        else:
            action_inputs = BatchFeature(
                {
            ...
            return self.action_head.get_action(
                [occ_hiddent_state.to(model_dtype),agent_hiddent_state.to(model_dtype),gp_hiddent_state.to(model_dtype), dream_occ_hiddent_state.to(model_dtype), dream_agent_hiddent_state.to(model_dtype)], action_inputs
            )
    ...
    def compute_loss(
        ...
        if self.training and self.grpo:
            ...
            return torch.nn.functional.l1_loss(
                predictions["pred_traj"], targets["trajectory"]
            )

    def get_optimizers(self) -> Union[Optimizer, Dict[str, LRScheduler]]:
        optimizer_cfg = DictConfig(
            dict(type="AdamW", lr=self._lr, weight_decay=1e-4, betas=(0.9, 0.95))
        )
        optimizer = build_from_configs(
            optim, optimizer_cfg, params=self.action_head.parameters()
        )

        if self.grpo:
            scheduler = WarmupCosLR(
                optimizer=optimizer, lr=self._lr, min_lr=0.0, epochs=10, warmup_epochs=0
            )
        else:
            scheduler = WarmupCosLR(
                optimizer=optimizer,
...
def make_sgdrive_config(
    ...
    *,
    ...
    """
    A factory function to create a SGDriveDiffusionPlannerConfig object.
    ...
    Returns:
        SGDriveDiffusionPlannerConfig: An instantiated and configured planner config object.
    """
    size = size.lower()
    if size == "small":
        diffusion_model_cfg = {
    ...
    config = SGDriveDiffusionPlannerConfig(
