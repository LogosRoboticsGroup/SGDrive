...
class Dataset_For_Traj(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        # return cameras, self._scene_loader.tokens[idx]
        ...
        agent_states, agent_labels, agent_names, future_velocities, box_2d = self._compute_agent_targets_3d(annotations, scene,scene.frames[frame_idx].cameras.cam_f0)

        return ego_statuses, cameras, future_trajectory, self._scene_loader.tokens[idx], agent_states, agent_labels, agent_names, future_velocities, box_2d
    
    def _compute_agent_targets_3d(self, annotations, scene, camera, future_frames=8) -> Tuple[torch.Tensor, torch.Tensor, List[str], torch.Tensor, torch.Tensor]:
        """
        Extracts 3D agent bounding boxes in ego coordinates, filtered by front camera FOV,
...
class Dataset_For_3d(torch.utils.data.Dataset):
    def __init__(
        self,
        scene_loader: SceneLoader,
        feature_builders: List[AbstractFeatureBuilder],
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        ...
        annotations_times = []
        for frame_idx in range(start_frame_idx, start_frame_idx + num_trajectory_frames + 1, 2):
            annotations = scene.frames[frame_idx].annotations 
            agent_states, agent_labels, agent_names, future_velocities, box_2d = self._compute_agent_targets_3d(annotations, scene,scene.frames[frame_idx].cameras.cam_f0)
            annotations_times.append({
        ...
        return ego_statuses, cameras, future_trajectory, self._scene_loader.tokens[idx], annotations_times
