...
def custom_collate_fn(
    ...
    occ_hidden_state = (
        rnn_utils.pad_sequence(
            [features["occ_hidden_state"] for features in features_list],
            batch_first=True,
            padding_value=0.0,
        )
        .clone()
    ...
    gp_hidden_state = (
        rnn_utils.pad_sequence(
            [features["gp_hidden_state"] for features in features_list],
            batch_first=True,
            padding_value=0.0,
        )
        .clone()
    ...
    agent_hidden_state = (
        rnn_utils.pad_sequence(
            [features["agent_hidden_state"] for features in features_list],
            batch_first=True,
            padding_value=0.0,
        )
        .clone()
    ...
    features = {
        ...
        "occ_hidden_state":occ_hidden_state,
        "agent_hidden_state":agent_hidden_state,
        "gp_hidden_state":gp_hidden_state,
        "dream_occ_hidden_state":dream_occ_hidden_state,
        "dream_agent_hidden_state":dream_agent_hidden_state,
    }
...
def main(cfg: DictConfig) -> None:
    ...
    train_dataloader = DataLoader(
        train_data, collate_fn=custom_collate_fn, **cfg.dataloader.params, shuffle=True
    )
    logger.info("Num training samples: %d", len(train_data))
    val_dataloader = DataLoader(
        val_data, collate_fn=custom_collate_fn, **cfg.dataloader.params, shuffle=False
    )
        ...
        **cfg.trainer.params,
        callbacks=[
            pl.callbacks.ModelCheckpoint(
                ...
                dirpath=os.getenv("SAVE_MODEL_PATH", ""),
                filename='checkpoint-{epoch:02d}-{val/loss_epoch:.4f}', 
                mode="min", 
                save_top_k=5, 
                every_n_epochs=int(os.getenv("EVERY_N_EPOCHS", 1)),
